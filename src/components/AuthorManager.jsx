...
export default function AuthorManager() {
  ...
  // --- NEW: State for auth token ---
  const [token, setToken] = useState(null);

  const API_URL = "https://xarwiz-admin-backend.onrender.com/api/admin/authors";

  useEffect(() => {
    // --- NEW: Get token on load ---
  ...
  // --- UPDATED: Pass token to functions ---
  ...
  const handleSubmit = async (e) => {
    ...
    // --- Prepare payload ---
    // If we are editing and password is blank, don't send it
    const payload = { ...formData };
    if (editingAuthor && !payload.password) {
    ...
    try {
      ...
      if (editingAuthor) {
        await axios.put(`${API_URL}/${editingAuthor._id}`, payload, config);
      } else {
      ...
      setEditingAuthor(null);
      fetchAuthors(token); // Pass token
    } catch (error) {
      console.error("Error saving author:", error.response?.data?.message || error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleEdit = (author) => {
    ...
    setFormData({
      ...
      // --- POPULATE EMAIL ---
      email: author.email || "",
      password: "", // Always leave password blank on edit
      // ---
      title: author.title || "",
      bio: author.bio || "",
      profileImage: author.profileImage || "",
      ...
      <div style={styles.card}>
        ...
        <form onSubmit={handleSubmit} style={styles.form}>
          ...
          <input
            type="password"
            name="password"
            placeholder={editingAuthor ? "New Password (optional)" : "Password *"}
            value={formData.password}
            onChange={handleChange}
            // Password is only required when creating a new author
          ...
          <div style={styles.list}>
            {authors.map((author) => (
              <div key={author._id} style={styles.listItem}>
                ...
                />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <h3 style={{ margin: "0 0 5px", color: "#1f2937" }}>
                    {author.displayName}
                  </h3>
                  {/* --- NEW: Show email --- */}
                  ...
                  <p style={{ fontSize: "14px", color: "#4b5563", margin: "5px 0 0" }}>
                    {author.title && <strong style={{color: "#1351d8"}}>{author.title}</strong>}
                    <br />
                    <span style={{whiteSpace: "pre-wrap", wordBreak: "break-word"}}>{author.bio}</span>
                  </p>
