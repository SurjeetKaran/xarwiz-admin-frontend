...
const GuestPostingManager = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!fetchedData) {
        ...
        // Ensure fetched data structure matches form state (especially arrays)
        setForm({
      ...
      setForm(defaultForm);
       showTempMessage("⚠️ Could not fetch data. Starting in create mode.", "warning");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleFeatureChange = (i, field, value) => {
    ...
    updated[i] = { ...updated[i], [field]: value }; // Ensure object update is correct
  ...
  const handleAddCard = () => {
     if (form.featureCards.length >= 6) { // Example limit: Max 6 cards
  ...
  const handleAddFaq = () => {
     if (form.faqs.length >= 10) { // Example limit: Max 10 FAQs
        showTempMessage("⚠️ Maximum number of FAQs reached.", "warning");
        return;
    }
    setForm({
      ...form,
      faqs: [...form.faqs, { id: `faq${Date.now()}`, question: "", answer: "" }], // Use timestamp for unique temp ID
    });
  };

  const handleCardDelete = (i) => {
    if (form.featureCards.length <= 1) { // Prevent deleting the last card
  ...
  const handleFaqDelete = (i) => {
     if (form.faqs.length <= 1) { // Prevent deleting the last FAQ
...
const handleDeleteConfirmed = async () => {
  ...
  const styles = {
    ...
    card: { // General card style for edit sections
    ...
    previewCard: {
      ...
      '&:last-child': { // Remove border for last item
        borderBottom: 'none',
        paddingBottom: '0.5rem', // Adjust padding
      },
       '&:first-child': { // Adjust padding for first item
        paddingTop: '0.5rem',
      },
    },
    previewImg: {
      ...
      borderRadius: "8px",
      objectFit: "cover",
      border: "1px solid #e5e7eb", // Add border to preview image
      flexShrink: 0,
    },
    ...
    actions: {
      ...
      gap: "10px",
      marginTop: "2rem", // Increased margin top for action buttons
      paddingTop: "1.5rem", // Padding top for separation
    ...
    dialogBox: {
      ...
      textAlign: 'center', // Center text in dialog
    },
     dialogTitle: {
        marginTop: 0,
        marginBottom: '1rem',
    ...
    error: {
      ...
      fontSize: "13px",
      marginTop: "-0.5rem", // Reduce space above error
      marginBottom: "0.5rem", // Space below error
    },
     loadingOverlay: { // Style for loading state
        position: 'absolute',
        inset: 0,
    ...
    successMessage: { // Styles for success
      backgroundColor: '#d1fae5', // Light Green
      color: '#065f46', // Dark Green
    },
    errorMessage: { // Styles for error/warning
  ...
  return (
    <div style={styles.container}>
      {loading && !editMode && <div style={styles.loadingOverlay}>Loading...</div>} {/* Show loading only when not editing */}
      <h2 style={styles.header}>Guest Posting Service Content</h2>

        {message && (
            <div style={{
                ...styles.message,
                ...(message.type === 'error' || message.type === 'warning' ? styles.errorMessage : styles.successMessage)
            }}>
                {message.text}
            </div>
      )}
       ...
       {/* Message when no data and not loading */}
        {!loading && !data && !editMode && (
             <div style={{...styles.card, textAlign: 'center'}}>
                 <p style={{ color: "#6b7280", fontSize: '1rem' }}>No Guest Posting content found.</p>
      ...
      {editMode && (
        <div style={styles.card}>
            {loading && <div style={styles.loadingOverlay}>Saving...</div>} {/* Loading indicator during save */}
             <h3 style={styles.subHeader}>{data ? "Edit Content" : "Create New Content"}</h3>
          <input
            ...
            {form.featureCards.map((c, i) => (
              <div key={i}>
                <div style={styles.cardEditItem}>
                  <div onClick={() => document.getElementById(`icon${i}`).click()} style={styles.imgPlaceholder}>
                    {c.iconPath ? <img src={c.iconPath} alt="preview" style={styles.imgPreviewSmall} /> : <span>Icon</span>}
                  </div>
                  <input type="file" id={`icon${i}`} style={{ display: "none" }} accept="image/*" onChange={(e) => handleIconUpload(i, e.target.files[0])} />
        ...
        {/* Edit FAQs */}
               ...
               <div key={i}>
                    <div style={styles.cardEditItem}>
                        <div style={{flex: 1, display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
                            ...
                            <textarea value={f.answer} onChange={(e) => handleFaqChange(i, "answer", e.target.value)} placeholder="FAQ Answer" style={{...styles.textarea, minHeight: '60px', marginBottom: 0}} />
                        </div>
         ...
         <div style={styles.actions}>
            ...
            {/* Show Cancel only if there's existing data to cancel editing */}
            {data && (
      ...
      {showDeleteConfirm && (
        <div style={styles.dialogOverlay}>
          <div style={styles.dialogBox}>
            ...
            <p style={styles.dialogText}>Are you sure you want to permanently delete all Guest Posting service content?</p>
...
export default GuestPostingManager;
