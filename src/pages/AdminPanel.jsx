...
export default function AdminPanel() {
  // --- UPDATED: Replaced 'loggedIn' with 'user' state ---
  const [user, setUser] = useState(null);
  const [activeTab, setActiveTab] = useState(null); // Start as null
  const navigate = useNavigate();

  // --- UPDATED: Logout now also removes user info ---
  const handleLogout = () => {
    localStorage.removeItem("token");
  ...
  // --- NEW: Check login status and role on component mount ---
  useEffect(() => {
    ...
    if (token && storedUser) {
      ...
      // If no token or user, force logout/redirect
      handleLogout();
    }
  ...
  // --- NEW: Show loading screen while user is being verified ---
  // This prevents flickering or showing the wrong default tab
  if (!user || !activeTab) {
    return (
      <div style={{...styles.page, justifyContent: 'center', alignItems: 'center', fontSize: '1.2rem', color: '#1351d8'}}>
        Loading Panel...
      </div>
    );
  }

  // --- UPDATED: getTabHeading now checks for role ---
  const getTabHeading = () => {
    // Authors only have one view
    if (user.role === "author") {
      return "Manage Blog Posts";
    }

    // Admin view (as before)
    switch (activeTab) {
      case "Dashboard":
        return "Welcome, Admin!";
      case "Testimonials":
  ...
  // --- UPDATED: renderContent now checks for role ---
  const renderContent = () => {
    // --- Security Check: Authors can ONLY see BlogManager ---
    if (user.role === "author") {
      return <BlogManager />;
    }

    // --- Admin-only content ---
    // (This code only runs if user.role is 'admin')
    switch (activeTab) {
      case "Dashboard":
        return (
          <div style={styles.dashboardContainer}>
            <h2 style={styles.dashboardHeading}>Welcome Admin</h2>
            <div style={styles.dashboardGrid}>
              {/* Index Edit Tools */}
              <div
                style={styles.dashboardCard}
                onMouseEnter={(e) => {
                  e.currentTarget.style.transform =
                    styles.dashboardCardHover.transform;
                  e.currentTarget.style.boxShadow =
                    styles.dashboardCardHover.boxShadow;
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.transform = "none";
                  e.currentTarget.style.boxShadow =
                    styles.dashboardCard.boxShadow;
                }}
              >
                <h3 style={styles.dashboardCardHeading}>Index Page Tools</h3>
                <ul style={styles.dashboardCardList}>
                  <li>Manage Testimonials</li>
                  <li>Update Stats section</li>
                  <li>Configure General Settings</li>
              ...
              >
                <h3 style={styles.dashboardCardHeading}>Blog Management</h3>
                <ul style={styles.dashboardCardList}>
                  <li>Add or edit blog posts</li>
                  <li>Manage authors</li>
                  <li>Organize categories & tags</li>
              ...
              >
                <h3 style={styles.dashboardCardHeading}>Service Content</h3>
                <ul style={styles.dashboardCardList}>
                  <li>Manage About Page</li>
                  <li>Edit Guest Posting Page</li>
                  <li>Edit Niche Edit Page</li>
  ...
  return (
    <div style={styles.page}>
      {/* Sidebar */}
      <div style={styles.sidebarWrapper}>
        {/* --- UPDATED: Pass the user's role to the Sidebar --- */}
        <Sidebar
          onLogout={handleLogout}
          onTabChange={setActiveTab}
          role={user.role} 
          activeTab={activeTab} // Pass activeTab as well
        />
      </div>
